...
.kdb.desc.iterators.over:"For monadic and dyadic functions, over returns the last item that scan produces.";

.kdb.desc.iterators.peach:"The binary keyword peach can be used as a mnemonic alternative. The following are equivalent.";

.kdb.desc.iterators.scan:"scan is a function that performs a running (cumulative) aggregation over a list. It is similar to fold (or reduce), but instead of returning a single result, it returns an intermediate result at each step.";