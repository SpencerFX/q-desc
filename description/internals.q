...
.kdb.desc.func.minus25Bang:"async broadcast - Broadcast data as an async msg to specified handles. The advantage of using -25!(handles;msg) over neg[handles]@\:msg is that -25!msg will serialize msg just once - thereby reducing CPU and memory load. Handles should be a vector of positive int or longs. msg will be serialized just once, to the lowest capability of the list of handles. I.e. if handles are connected to a mix of versions of kdb+, it will serialize limited to the types supported by the lowest version. If there is an error, no messages will have been sent, and it will return the handle whose cap caused the error. Just as with neg[handles]@\:msg, -25!x queues the msg as async on those handles - they don't get sent until the next spin of the main loop, or are flushed with neg[handles]@\:(::).";
...
.kdb.desc.func.minus27Bang:"IEEE754 precision format - Where x is an int atom, y is a float returns y as a string or strings formatted as a float to x decimal places. (Since V3.6 2018.09.26.) It is atomic and doesn't take \P into account. For example: q)-27!(3i;0 1+123456789.4567) 123456789.457, 123456790.457";
