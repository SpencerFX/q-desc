/ ==================================================
/ command-line arg example capture functions
/ ==================================================

/ --------------------------------------------------
/ map q type code to readable label
/ --------------------------------------------------
.test.args.typeLabel:{[typeCode]
  code:`int$typeCode;
  $[code=-19;"timeAtom";
    code=-18;"secondAtom";
    code=-17;"minuteAtom";
...
/ --------------------------------------------------
.test.args.preview:{[objVal]
  @[{$[99h=type x;"table[",string count x,"] ",.Q.s1 x;.Q.s1 x]};objVal;{"<unprintable>"}]
 };

/ --------------------------------------------------
/ resolve example function symbol safely
/ --------------------------------------------------
.test.args.resolve:{[fnSym]
  @[value;fnSym;{(::)}]
 };

/ --------------------------------------------------
/ test whether example function exists
/ --------------------------------------------------
.test.args.exists:{[fnSym]
  fnVal:.test.args.resolve fnSym;
  not fnVal~(::)
 };

/ --------------------------------------------------
/ execute example safely
/ --------------------------------------------------
.test.args.run:{[fnVal]
  ok:1b;
  errText:"";
...
/ --------------------------------------------------
/ capture example execution metadata
/ --------------------------------------------------
.test.args.capture:{[label;fnSym]
  fnExists:.test.args.exists fnSym;
  fnVal:.test.args.resolve fnSym;
  ...
  resultTypeName:$[ok;.test.args.typeLabel resultTypeCode;""];
  resultPreview:$[ok;.test.args.preview resultVal;""];
