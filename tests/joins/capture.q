/ ==================================================
/ join capture functions
/ ==================================================

/ --------------------------------------------------
/ map q type code to readable label
/ --------------------------------------------------
.test.join.typeLabel:{[typeCode]
  code:`int$typeCode;
  $[code=-19;"timeAtom";
    code=-18;"secondAtom";
    code=-17;"minuteAtom";
    code=-16;"timespanAtom";
    code=-15;"datetimeAtom";
    code=-14;"dateAtom";
    code=-13;"monthAtom";
    code=-12;"timestampAtom";
    code=-11;"symbolAtom";
    code=-10;"charAtom";
    code=-9;"floatAtom";
    code=-8;"realAtom";
    code=-7;"longAtom";
    code=-6;"intAtom";
    code=-5;"shortAtom";
    code=-4;"byteAtom";
    code=-2;"guidAtom";
    code=-1;"booleanAtom";
    code=0;"generalList";
    code=1;"booleanList";
    code=2;"guidList";
    code=4;"byteList";
    code=5;"shortList";
    code=6;"intList";
    code=7;"longList";
    code=8;"realList";
    code=9;"floatList";
    code=10;"charList";
    code=11;"symbolList";
    code=12;"timestampList";
    code=13;"monthList";
    code=14;"dateList";
    code=15;"datetimeList";
    code=16;"timespanList";
    code=17;"minuteList";
    code=18;"secondList";
    code=19;"timeList";
    code=98;"dictionary";
    code=99;"table";
    code=100;"lambda";
    code=104;"projection";
    code=105;"primitive";
    "unknown(",string code,")"]
 };

/ --------------------------------------------------
/ generate safe printable preview of object
/ --------------------------------------------------
.test.join.preview:{[objVal]
  @[{$[99h=type x;"table[",string count x,"] ",.Q.s1 x;.Q.s1 x]};objVal;{"<unprintable>"}]
 };

/ --------------------------------------------------
/ normalize args into general list
/ --------------------------------------------------
.test.join.normalizeArgs:{[argVals]
  $[0h=type argVals;argVals;enlist argVals]
 };

/ --------------------------------------------------
/ test whether object is keyed table
/ --------------------------------------------------
.test.join.isKeyedTable:{[objVal]
  99h=type objVal
 };

/ --------------------------------------------------
/ capture join execution metadata and result
/ --------------------------------------------------
.test.join.capture:{[joinName;joinFunc;argVals]
  argList:.test.join.normalizeArgs argVals;
  ok:1b;
  errText:"";

  resultVal:.[joinFunc;argList;{ok::0b;errText::string x;(::)}];

  argTypeCodes:type each argList;
  argTypeNames:.test.join.typeLabel each argTypeCodes;
  argPreviews:.test.join.preview each argList;
  argKeyedFlags:.test.join.isKeyedTable each argList;
  resultTypeCode:$[ok;type resultVal;0Nh];
  resultTypeName:$[ok;.test.join.typeLabel resultTypeCode;""];
  resultPreview:$[ok;.test.join.preview resultVal;""];

  ([] joinName:enlist joinName;
     argCount:enlist count argList;
     argTypeCodes:enlist argTypeCodes;
     argTypeNames:enlist argTypeNames;
     argPreviews:enlist argPreviews;
     argKeyedFlags:enlist argKeyedFlags;
     ok:enlist ok;
     resultTypeCode:enlist resultTypeCode;
     resultTypeName:enlist resultTypeName;
     resultPreview:enlist resultPreview;
     errorText:enlist errText;
     resultValue:enlist resultVal)
 };